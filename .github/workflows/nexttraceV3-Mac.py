...
# ========== ping 3 次取平均，返回整数（macOS 兼容）==========
def measure_ping_delay(ip, timeout_sec=3):
    try:
        timeout_ms = int(timeout_sec * 1000)
        # macOS ping 使用 -c 1 -W 超时（毫秒），但 -W 是每次等待的毫秒，我们发送3次
        cmd = ["ping", "-c", "3", "-W", str(timeout_ms), ip]
        # 在 macOS 下无需隐藏窗口，直接执行
        ...
        # 兼容 macOS 输出格式： time=1.23 ms 或 time=1.23ms
...
class NextTraceGUI:
    ...
    def _get_exe_path(self):
        # macOS 下 nexttrace 可能没有扩展名，或为 nexttrace 可执行文件
        bundled = resource_path("nexttrace")
        if os.path.isfile(bundled):
            return bundled
        # 也尝试 .exe 后缀（兼容 Windows 打包）
        bundled_exe = resource_path("nexttrace.exe")
        if os.path.isfile(bundled_exe):
            return bundled_exe
        local = os.path.join(BASE_DIR, "nexttrace")
        if os.path.isfile(local):
            return local
        local_exe = os.path.join(BASE_DIR, "nexttrace.exe")
    ...
    def start_test(self):
        ...
        if not self.exe_path or not os.path.isfile(self.exe_path):
            messagebox.showerror("错误", "未找到 nexttrace 可执行文件，请确保文件存在")
            return
        if self.matcher is None:
